...
void BPlusTree::displayNode(Node *btreenode)
{
    ...
    int ctr = 0;
    std::cout << ">";
    int keys = btreenode->curKeyCount;
    while (ctr < keys)
    {
...
void BPlusTree::display(Node *flagDisk, int lvl)
{
    ...
    Node *flag = (Node *)index->read(flagMM, nodeSize);
    ...
    if (flag != nullptr)
    {
        ...
        {
            while (ctr < flag->curKeyCount + 1)
            {
                Node *mmNode = (Node *)index->read(flag->pointers[ctr], nodeSize);
...
void BPlusTree::displayLL(Address llHead)
{
    ...
    {
        displayLL(head->pointers[head->curKeyCount]);
    }
}