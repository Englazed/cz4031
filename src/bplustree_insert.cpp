...
void BPlusTree::insert(Address address, int key)
{
    ...
    if (rootAddress == nullptr)
    {
        ...
        Address LLNodeAddress = index->write((void *)LLNode, nodeSize);
        ...
        rootAddress = index->write(root, nodeSize).blockAddress;
    }
    else
    {
        Node *cursor = root;
        ...
        while (cursor->isLeaf == false)
        {
            ...
            for (int i = 0; i < cursor->curKeyCount; i++)
            {
                ...
                {
                    Node *mainMemoryNode = (Node *)index->read(cursor->pointers[i], nodeSize);
                ...
                {
                    Node *mainMemoryNode = (Node *)index->read(cursor->pointers[i + 1], nodeSize);
        ...
        if (cursor->curKeyCount < maxKeyCount)
        {
            ...
            else
            {
                ...
                index->write(cursor, nodeSize, cursorOriginalAddress);
            }
        }
        ...
        else
        {
            ...
            Address newLeafAddress = index->write(newLeaf, nodeSize);
            ...
            if (cursor == root)
            {
                ...
                Address newRootAddress = index->write(newRoot, nodeSize);
...
void BPlusTree::insertInternal(int key, Node *cursorDiskAddress, Node *childDiskAddress)
{
    ...
    Node *cursor = (Node *)index->read(cursorAddress, nodeSize);
    ...
    Node *child = (Node *)index->read(childAddress, nodeSize);
    ...
    else
    {
        ...
        Address cursorAddress{cursorDiskAddress, 0};
        index->write(cursor, nodeSize, cursorAddress);
        Address newInternalDiskAddress = index->write(newInternal, nodeSize);
        ...
        if (cursor == root)
        {
            ...
            newRoot->curKeyCount = 1;
            root = newRoot;
            Address newRootAddress = index->write(root, nodeSize);
            rootAddress = newRootAddress.blockAddress;
        }
        else
        {
...
{
    Node *head = (Node *)index->read(LLHead, nodeSize);
    ...
    if (head->curKeyCount < maxKeyCount)
    {
        ...
        head->curKeyCount++;
        LLHead = index->write((void *)head, nodeSize, LLHead);
