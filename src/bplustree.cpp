...
BPlusTree::BPlusTree(std::size_t blockSize, DiskStorage *disk, DiskStorage *index)
{
...
Node *BPlusTree::findParent(Node *cursorDiskAddress, Node *childDiskAddress, int lowerBoundKey)
{
...
int BPlusTree::getLevels()
{
    ...
    while (!cursor->isLeaf)
    {
        cursor = (Node *)index->read(cursor->pointers[0], nodeSize);
        levelsCount++;
    }
