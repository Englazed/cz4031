...
{
    int numNodes = index->getAllocatedBlocks();
    ...
    else
    {
        ...
        if (leftSibling >= 0)
        {
            Node *leftNode = (Node *)index->read(parent->pointers[leftSibling], nodeSize);
        ...
        {
            Node *rightNode = (Node *)index->read(parent->pointers[rightSibling], nodeSize);
...
void BPlusTree::removeInternal(int key, Node *cursorDiskAddress, Node *childDiskAddress)
{
    ...
    if (cursor == root)
    {
        if (cursor->curKeyCount == 1)
        {
            if (cursor->pointers[1].blockAddress == childDiskAddress)
            {
                ...
                root = (Node *)index->read(cursor->pointers[0], nodeSize);
            ...
            {
                index->deallocate(childAddress, nodeSize);
                root = (Node *)index->read(cursor->pointers[1], nodeSize);
    ...
    int leftSibling, rightSibling;
    Address parentAddress{parentDiskAddress, 0};
    Node *parent = (Node *)index->read(parentAddress, nodeSize);
...
{
    Node *head = (Node *)index->read(LLHeadAddress, nodeSize);
